...
```
Роутер R2 больше не является соседом R1 и R3, т.к. не рассылает маршруты и из-за этого у данных роутеров нет маршрута к сети 192.168.2.0/24.
<details>
...
```
Теперь роутеры R1 и R3 узнают про маршрут до 192.168.2.0/24
